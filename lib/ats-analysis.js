...
const SCHEMA = {
  ...
  properties: {
    ...
    matched_requirements:{type:"array",items:{type:"object",properties:{requirement:{type:"string"},resume_evidence:{type:"string"}},required:["requirement","resume_evidence"],additionalProperties:false},description:"When a target job is supplied, requirements supported by explicit resume evidence; otherwise empty."},
    missing_requirements:{type:"array",items:{type:"object",properties:{requirement:{type:"string"},how_to_address:{type:"string"}},required:["requirement","how_to_address"],additionalProperties:false},description:"When a target job is supplied, requirements not evidenced in the resume. Never suggest adding an unearned qualification; otherwise empty."},
    job_recommendations: {
      type: "array",
  ...
  required: ["ats_score", "verdict", "strengths", "gaps", "formatting_flags", "keywords_to_add", "rewrite_tips", "matched_requirements", "missing_requirements", "job_recommendations"],
  additionalProperties: false,
};

const SYSTEM = `You are a resume reviewer and ATS specialist for The Career Architect.
Assess ATS readability and recruiter appeal using only the resume supplied. If a target job description is provided, score alignment to that job and distinguish matched requirements with explicit resume evidence from requirements not evidenced. If no target job is provided, leave both requirement arrays empty and score general ATS readiness. Do not imply the score is a measured hiring probability.
