...
export function reportSections(record){
  ...
  return [["Candidate",`${record.full_name} | ${record.field||"Career assessment"}`],["ATS readiness estimate",`${a.ats_score}/100 - ${a.verdict}\nThis is an estimate of résumé readiness, not a hiring probability.`],["Career assessment",g.overview],["Strengths",a.strengths],["Gaps and improvements",[...a.gaps,...a.rewrite_tips]],["Jobs to apply for",roles],["Action plan",g.action_plan],["Interview preparation",[...g.interview_questions.map((q,i)=>`${i+1}. ${q.question}\nWhy asked: ${q.why_asked}\nAnswer guidance: ${q.answer_guidance}`),...g.interview_tips.map(x=>`Tip: ${x}`)]]];
}
