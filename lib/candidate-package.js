...
const schema = {
  type:"object", properties:{
    ...
    follow_up_plan:{type:"array",items:{type:"object",properties:{when:{type:"string"},action:{type:"string"}},required:["when","action"],additionalProperties:false}},
    email_templates:{type:"array",items:{type:"object",properties:{occasion:{type:"string"},subject:{type:"string"},body:{type:"string"}},required:["occasion","subject","body"],additionalProperties:false}}
  }, required:["overview","action_plan","tailored_resume","resume_changes","interview_questions","interview_tips","interview_preparation","follow_up_plan","email_templates"], additionalProperties:false
};
export async function generatePackage(intake, ats){
  ...
  const msg=await ai.messages.create({model:"claude-opus-4-8",max_tokens:7000,
    output_config:{effort:"medium",format:{type:"json_schema",schema}},
    system:"You are a careful career strategist, résumé writer and interview coach. Use only verified facts in the intake and résumé. Never invent employers, dates, metrics, degrees, certifications, or achievements. Tailor the résumé to realistic target roles, with clear ATS-friendly sections. Give a detailed, actionable career report and 6-8 interview questions with specific answer guidance; never present an invented answer as the candidate's own experience. Include a practical interview preparation checklist covering role research, evidence-based examples, questions to ask, and post-interview reflection. Include a respectful follow-up schedule after applying and interviewing, plus two concise customizable email templates: application check-in and post-interview thank-you/check-in. Use [Company], [Role], and [Interviewer] placeholders where needed. Do not suggest repeated unsolicited messages. ATS score is an estimate of document readiness, not a hiring probability. Avoid promises of job placement or unverified sponsorship claims.",
...
export function reportSections(record){
  ...
  return [["Candidate",`${record.full_name} | ${record.field||"Career assessment"}`],["ATS readiness estimate",`${a.ats_score}/100 - ${a.verdict}\nThis is an estimate of résumé readiness from the supplied text, not a hiring probability. Match to a specific vacancy requires its job description.`],["Career assessment",g.overview],["ATS strengths",a.strengths||[]],["ATS gaps",a.gaps||[]],["Formatting and readability flags",(a.formatting_flags||[]).length?a.formatting_flags:["No specific formatting issue was visible in the supplied text; check the original file before applying."]],["Supported keywords to use",(a.keywords_to_add||[]).length?a.keywords_to_add:["No additional verified keywords identified."]],["Specific résumé improvements",a.rewrite_tips||[]],["Jobs to apply for",roles],["Action plan",g.action_plan||[]]];
}
export function prepSections(record){
  const guide=record.data.package.guide;
  const cadence=(guide.follow_up_plan||[]).map(x=>`${x.when}: ${x.action}`);
  const templates=(guide.email_templates||[]).map(x=>`${x.occasion}\nSubject: ${x.subject}\n${x.body}`);
  return [["Candidate",record.full_name],["Interview preparation checklist",guide.interview_preparation||[]],["Interview questions",(guide.interview_questions||[]).map((q,i)=>`${i+1}. ${q.question}\nWhy asked: ${q.why_asked}\nAnswer guidance: ${q.answer_guidance}`)],["Preparation tips",guide.interview_tips||[]],["Follow-up and check-in plan",cadence],["Email templates to customize",templates],["Practice with your AI coach","Your private practice link is included in the email. Use your real experience when answering."]];
}
