...
export async function sendMaterials(sb,record,{manual=false}={}){
  ...
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{
    ...
    html:`<div style="font-family:Arial,sans-serif;color:#13213b;max-width:600px"><h2>Your career materials are ready</h2><p>Hi ${safeName(record.full_name)},</p><p>Attached are your career report, tailored résumé, and interview preparation as three separate PDFs. Your report includes an ATS readiness breakdown, realistic job types, and a clear action plan. The interview guide includes preparation steps, a follow-up schedule, and email templates.</p><p><a href="${url}">Practice interviews with your private AI coach</a></p><p>Keep this link private. Reply to this email if you need a change.</p></div>`,
    attachments
  })});
...
export async function processAssessment(id,sb=getServiceClient()){
  ...
  if(data.report_tier!=="paid")return {skipped:true,status:"not_included"};
