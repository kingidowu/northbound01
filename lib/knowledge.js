...
export async function saveKnowledge(sb, { source_type, title, summary, extraction, tags }) {
  if (!sb) return;
  try {
    const { error } = await sb.from("knowledge_library").insert({ source_type, title, summary, extraction, tags });
    if (error) throw error;
  } catch (e) { console.error("saveKnowledge failed:", e); throw e; }
}
...
export async function ingest(sb, source_type, title, input, { atsAnalysis = null } = {}) {
  try {
    const extraction = await extractKnowledge(input);
    if (atsAnalysis) extraction.ats = atsAnalysis;
