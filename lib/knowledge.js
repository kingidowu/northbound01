...
export async function extractKnowledge(text) {
  const msg = await anthropic.messages.create({
    ...
    system: "You extract reusable, structured career knowledge from a resume or career-assessment submission, for a careers platform focused on IT, healthcare, and cloud computing across the US and Canada. Be precise and factual.",
    messages: [{ role: "user", content: `Extract knowledge from this submission:\n\n${text.slice(0, 16000)}` }],
  });
...
export async function ingest(sb, source_type, title, text) {
  try {
